...
// ── CSRF ──────────────────────────────────────────────────────
// Uses a stateless HMAC double-submit approach so the token can be
// verified by ANY handler (Auth/, api/, etc.) without needing the
// same PHP session that generated it. This avoids SameSite cookie
// issues when forms POST across directories on localhost.
//
// Token format:  <random_nonce>.<HMAC-SHA256(nonce, secret)>
// The secret is derived from a stable per-install key (APP_SECRET)
// or falls back to a value stored in the UMDC_SES cookie itself.

define('CSRF_SECRET', defined('APP_SECRET') ? APP_SECRET : 'umdc-csrf-secret-change-in-production');

function csrf_token(): string {
    // Check for an existing valid token in the session first
    umdc_session_start();
    if (!empty($_SESSION['csrf_token']) && _csrf_token_valid($_SESSION['csrf_token'])) {
        return $_SESSION['csrf_token'];
    }
    // Generate a new stateless HMAC token
    $nonce  = bin2hex(random_bytes(16));
    $sig    = hash_hmac('sha256', $nonce, CSRF_SECRET);
    $token  = $nonce . '.' . $sig;
    $_SESSION['csrf_token'] = $token;
    return $token;
}

function _csrf_token_valid(string $token): bool {
    $parts = explode('.', $token, 2);
    if (count($parts) !== 2) return false;
    [$nonce, $sig] = $parts;
    $expected = hash_hmac('sha256', $nonce, CSRF_SECRET);
...
function csrf_verify(): bool {
    ...
    // Primary: validate the HMAC signature — no session required
    if (_csrf_token_valid($token)) return true;
    // Fallback: session-based comparison (belt-and-suspenders)
    umdc_session_start();
    $sessToken = $_SESSION['csrf_token'] ?? '';
    return $sessToken && hash_equals($sessToken, $token);
}
