...
if (isLoggedIn()) {
    ...
    $dest = match($role) { "admin" => "/dashboard/admin.php", "organization" => "/dashboard/organization.php", default => "/dashboard/user.php" }; header("Location: $dest" . $sess);
    exit;
}
...
csrf_check(); // Verify CSRF token from login form

// Rate limit by IP
$ip = $_SERVER['HTTP_X_FORWARDED_FOR'] ?? $_SERVER['REMOTE_ADDR'] ?? 'unknown';
if (!rateLimit("login:{$ip}", 10, 300)) {
    header('Location: /public/login.php?error=rate_limit');
    exit;
}
